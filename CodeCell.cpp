...
void CodeCell::Init(uint16_t sense_motion) {
  ...
  timerAlarm(cctimer, 16000, true, 0);     /*Set alarm to trigger every 16ms with auto-reload*/
...
void CodeCell::Sleep(uint16_t sleep_sec) {
  ...
  Serial.printf("Sleeping for %0.1f\n", sleep_sec);
  ...
  if (cc_timeflag) {
    ...
    if ((battery_voltage > USB_VOLTAGE) || (digitalRead(0) == 0)) {
      ...
      if (digitalRead(0) == 0) {
        ...
        /*USB Cabel connected without battery - Run application  LED_Breathing(LED_COLOR_BLUE);*/
...
void CodeCell::Motion_RotationRead(float &roll, float &pitch, float &yaw) {
  ...
  if (Motion.getSensorEventID() == SENSOR_REPORTID_ROTATION_VECTOR) {
    ...
    roll = atan2(2.0 * (Rr * Ri + Rj * Rk), 1.0 - 2.0 * (Ri * Ri + Rj * Rj));  /* ROLL */
    pitch = atan2(2.0 * (Rr * Rj - Rk * Ri), 1.0 - 2.0 * (Rj * Rj + Ri * Ri)); /* PITCH */
    yaw = atan2(2.0 * (Rr * Rk + Ri * Rj), 1.0 - 2.0 * (Rj * Rj + Rk * Rk));   /* YAW */
  }
  Wire.endTransmission();
}
